...
function App() {
  ...
  const loadDishes = async () => {
    ...
    if (fetchError) {
      ...
      if (errorMessage.includes('category') || errorMessage.includes('relation') || errorMessage.includes('column')) {
        ...
        setError(`Failed to load cocktails: ${errorMessage}. Please check your Supabase connection.`)
      }
  ...
  const handleSaveDish = async (dishData) => {
    ...
    try {
      ...
      if (saveError) {
        setError(`Failed to save cocktail: ${saveError.message || 'Please check your Supabase connection and try again.'}`)
        ...
        setError(`Failed to save cocktail: ${err.message || 'An unexpected error occurred.'}`)
  ...
  const handleDeleteDish = async (dishId) => {
    ...
    if (deleteError) {
      setError('Failed to delete cocktail. Please try again.')
    ...
    try {
      ...
      for (const ingredient of ingredients) {
        ...
        const templateData = {
          ...
          total_yield: 1,
          yield_unit: ingredient.unit || 'fl oz',
          ingredients: [{
            name: ingredient.name,
            unit_cost: ingredient.price,
            quantity: 1,
            unit: ingredient.unit || 'fl oz',
            package_cost: null,
            package_size: null,
            package_unit: null
          }]
        ...
        {loading ? (
          <div className="text-center py-20">
            ...
            <div className="max-w-md mx-auto">
              ...
              <h3 className="text-2xl font-semibold text-gray-900 mb-2">No cocktails yet</h3>
