...
const TemplateForm = ({ template, onSave, onCancel, onDelete }) => {
  ...
  const [yieldUnit, setYieldUnit] = useState('fl oz')
  ...
  useEffect(() => {
    if (template) {
      ...
      setYieldUnit(template.yield_unit || 'fl oz')
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      <div className="bg-white/90 backdrop-blur-sm p-8 rounded-2xl shadow-xl border-2 border-purple-100">
        <h2 className="text-3xl font-black text-gray-900 mb-6">
          {template ? 'Edit Template' : 'Create Spirit/Mixer Template'}
        </h2>
      ...
      <div className="bg-white/90 backdrop-blur-sm p-8 rounded-2xl shadow-xl border-2 border-purple-100">
        <h3 className="text-xl font-bold text-gray-900 mb-6">Spirits & Mixers</h3>
        <IngredientForm 
