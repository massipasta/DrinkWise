...
export const exportDishToPDF = (dish) => {
  ...
  doc.text(`Cost per Drink: $${plateCost.toFixed(2)}`, 25, yPos)
  yPos += 7
  
  if (sellingPrice > 0) {
    doc.text(`Selling Price: $${sellingPrice.toFixed(2)}`, 25, yPos)
    yPos += 7
    doc.text(`Pour Cost: ${foodCostPercentage.toFixed(1)}%`, 25, yPos)
    yPos += 7
    doc.text(`Profit per Drink: $${grossProfit.toFixed(2)}`, 25, yPos)
    yPos += 7
  ...
  // Spirits & Mixers section
  if (dish.ingredients && dish.ingredients.length > 0) {
    doc.setFont('helvetica', 'bold')
    doc.setFontSize(14)
    doc.text('Spirits & Mixers', 20, yPos)
    yPos += 10
    ...
    dish.ingredients.forEach((ingredient, index) => {
      ...
      doc.text(name, 20, yPos)
      doc.text(`${quantity} ${ingredient.unit || 'fl oz'}`, 80, yPos)
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      `Generated by DrinkWise - Page ${i} of ${pageCount}`,
      105,
...
export const exportDishToCSV = (dish) => {
  ...
  // Build CSV content
  let csv = `Cocktail: ${dish.name}\n`
  csv += `Generated: ${new Date().toLocaleString()}\n\n`
  
  // Summary section
  csv += `Cost Summary\n`
  csv += `Cost per Drink,$${plateCost.toFixed(2)}\n`
  if (sellingPrice > 0) {
    csv += `Selling Price,$${sellingPrice.toFixed(2)}\n`
    csv += `Pour Cost,${foodCostPercentage.toFixed(1)}%\n`
    csv += `Profit per Drink,$${grossProfit.toFixed(2)}\n`
  } else {
  ...
  if (dish.ingredients && dish.ingredients.length > 0) {
    ...
    dish.ingredients.forEach((ingredient) => {
      ...
      csv += `${name},${quantity},${ingredient.unit || 'fl oz'},$${unitCostPerUnit.toFixed(4)},$${unitCost.toFixed(2)}\n`
    })
  }
